...
func (t *CacheTransport) RoundTripDummy(req *http.Request) (resp *http.Response, err error) {
	...
	if cacheable {
		...
		if err == nil {
			_ = t.Cache.Set(req.Context(), cacheKey, respBytes, cacheExpires(resp.Header))
		}
	} else {
		_ = t.Cache.Del(req.Context(), cacheKey)
	}
...
func (t *CacheTransport) RoundTripRFC2616(req *http.Request) (resp *http.Response, err error) {
	...
	control := parseCacheControl(resp.Header)
	if cacheable && canStore(parseCacheControl(req.Header), control) {
		...
		timeout := cacheExpires(resp.Header)
		...
		case http.MethodGet:
			...
			resp.Body = &cachingReadCloser{
				...
				OnEOF: func(r io.Reader) {
					...
					respBytes, err := httputil.DumpResponse(&resp, true)
					if err == nil {
						_ = t.Cache.Set(req.Context(), cacheKey, respBytes, timeout)
					}
				},
			}
		default:
...
// cacheExpires returns the cache timeout duration
func cacheExpires(headers http.Header) time.Duration {
	var timeout time.Duration
	if ctrl := headers.Get("Cache-Control"); ctrl != "" {
		for _, part := range strings.Split(ctrl, ",") {
			...
			if strings.Trim(key, " ") == "max-age" {
				timeout, _ = time.ParseDuration(strings.Trim(value, " ") + "s")
				break
			}
		}
	} else {
		date, err := parserDate(headers)
		if err != nil {
			return 0
		}
		var exp time.Time
		if expires := headers.Get("Expires"); expires != "" {
			exp, err = time.Parse(time.RFC1123, expires) //nolint:govet
		} else if lastModified := headers.Get("Last-Modified"); lastModified != "" {
			exp, err = time.Parse(time.RFC1123, lastModified) //nolint:govet
		} else {
			return 0
		}
		if err == nil {
			timeout = date.Sub(exp)
		}
	}
	return timeout
}
